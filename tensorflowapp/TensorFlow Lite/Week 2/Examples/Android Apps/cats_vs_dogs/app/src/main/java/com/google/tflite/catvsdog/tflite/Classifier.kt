...
class Classifier(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    ...
    init {
        //  Set the Interpreter's Option
        val options = Interpreter.Options()
        options.setNumThreads(5)
        options.setUseNNAPI(true) // use network API if avaiable
    ...
    private fun loadModelFile(assetManager: AssetManager, modelPath: String): MappedByteBuffer {
        // Get the file descriptor of model
        val fileDescriptor = assetManager.openFd(modelPath)
        // Read the model file's channel
        val inputStream = FileInputStream(fileDescriptor.fileDescriptor)
        val fileChannel = inputStream.channel
        val startOffset = fileDescriptor.startOffset
        val declaredLength = fileDescriptor.declaredLength
        //Load the TFLine model as
